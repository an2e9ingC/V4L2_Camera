...
2.	之前在板子上运行时,出现在ioctl(fd, VIDIOC_DQBUF, &tmp_buf)出现阻塞
原因:	初始化:request_buffers-->mmap与 入队 VIDIOC_QBUF 分开了, 应该在映射后立即入队,而不是分开处理,否则入队的不是申请映射的缓存;
    
3.	在开发板生成的yuyv格式文件, 不能通过YUYV查看器直接查看(linux环境下)
